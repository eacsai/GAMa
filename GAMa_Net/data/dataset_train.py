...
class bdd_vgl(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_hn, sat_image_hn = self.get_sample_hn(video_name, int(num_frames))

        # label_hn = idx%self.batch_size + self.batch_size

        videos = torch.stack([video, video_hn], dim=0)
        images = torch.stack([sat_image, sat_image_hn], dim=0)
        # labels = torch.stack([label, label_hn], dim=0)

        return videos, images# , label, label_hn

        #return video, sat_image, label
    ...
    def get_sample_hn(self, video_name, num_frames):
        ...
        if self.v_transform is not None:
            self.v_transform.randomize_parameters()
