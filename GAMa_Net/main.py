...
def parse_option():
    ...
    parser.add_argument('--learning_rate', type=float, default=8e-5,
                        help='learning rate')
    parser.add_argument('--lr_decay_epochs', type=str, default='100,150,250',
    ...
    parser.add_argument('--data_folder', type=str, default='/public/home/shiyj2-group/video_localization/GAMa_dataset/', help='path to custom dataset')
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (videos, images) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            # compute loss
            v_features, i_features = model(videos, images)
            # f1, f2 = torch.split(features, [bsz, bsz], dim=0)
            # features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
            features = torch.cat([v_features.unsqueeze(1), i_features.unsqueeze(1)], dim=1)
            labels = None
