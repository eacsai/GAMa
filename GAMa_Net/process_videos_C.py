...
def set_model(opt):
    model = TestResNet(name=opt.model, feat_dim=opt.feat_dim)
    # print(model)
...
def get_v_embeddings(test_loader, model, opt):
    ...
    embeddings = []
    with torch.no_grad():
        for idx, videos in enumerate(test_loader):
            ...
            v_features = model(videos)
            # print(v_features[0])
            embeddings.extend(v_features.cpu().numpy())
...
def main():
    ...
    # build data loader
    test_loader = set_loader_test(opt)

    # build model
    model = set_model(opt)

    v_embeddings = get_v_embeddings(test_loader, model, opt)

    base_path = os.path.split(opt.weights)[0]
    #embeddings_name = os.path.join(base_path, 'embeddings_videos_C_full.npy')
    embeddings_name = os.path.join(base_path, 'embeddings_videos_C.npy')
    #embeddings_name = os.path.join(base_path, 'embeddings_videos_C_finetuning.npy')

    np.save(embeddings_name, v_embeddings)
